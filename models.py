...
class Stock(Base):
    ...
    book_s = relationship(Book, backref="stock")
    shop_s = relationship(Shop, backref="stock")
...
class Sale(Base):
    ...
    stock_s = relationship(Stock, backref="sale")

    def __str__(self):
        return f'Магазин {self.id}: {self.prise}: {self.date_sale}: {self.id_stock}: {self.count}'
